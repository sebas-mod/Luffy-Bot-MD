...
if (!args[0]) return m.reply('☁️ Ingresa la cantidad de *☠️ Calaveras* que deseas Retirar.')
if (args[0] == 'all') {
let count = parseInt(user.bank)
user.bank -= count * 1
user.corazones += count * 1
await m.reply(`☁️ Retiraste *${count} ☠️ Calaveras* del Banco.`)
return !0
}
if (!Number(args[0])) return m.reply('☁️ La cantidad deve ser un Numero.')
let count = parseInt(args[0])
if (!user.bank) return m.reply('No tienes *☁️ Corazones* en el Banco.')
