...
let sortedLim = users.map(toNumber('calavera')).sort(sort('calavera'))
...
││ ${sortedLim.slice(0, len).map(({ jid, corazones }, i) => `${i + 1}. ${participants.some(p => jid === p.jid) ? `(${conn.getName(jid)}) wa.me/` : '@'}${jid.split`@`[0]} *${calavera} ☠️*`).join`\n││ `}
│╰────────────────···
╰───────────═┅═──────────
