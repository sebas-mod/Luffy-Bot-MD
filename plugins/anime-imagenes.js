...
let handler = async (m, { command, conn, usedPrefix }) => {
  ...
  try {
    ...
    const res = (await axios.get(`https://raw.githubusercontent.com/Guru322/api/Guru/BOT-JSON/anime-${command}.json`)).data;
    ...
    const results = await Promise.all(images.map(async (imageUrl, index) => {
      ...
      return {
        ...
        footer: proto.Message.InteractiveMessage.Footer.fromObject({ text: '*[ Luffy-Bot BY Sebas_MD ]*' }),
