...
// Manejador para las interacciones de los botones
handler.action = async (m, { conn, action }) => {
  if (action.buttonId.startsWith('anotar ')) {
    const partidaId = action.buttonId.split(' ')[1];
    const quien = m.sender;
    const { name } = global.db.data.users[quien];
